...
Public Class SyntacticAnalysis
    ...
    Public Sub Conversion(ByVal strPath As String, ByVal outputFilePath As String, Optional columnTitleFlg As Boolean = False)
        ...
        Using cWriter = New System.IO.StreamWriter(OUT_FILE, True, System.Text.Encoding.GetEncoding("shift_jis"))
            ...
            If System.IO.Path.GetExtension(strPath) <> ".wtj" Then
                ...
                Return
            ElseIf Me.RECORD.C3_WTJ_FILE_NAME.StartsWith("接続情報登録_") Then
                Me.CONSOLEWRITE("SyntacticAnalysys.Conversion : ", "「接続情報登録_」で始まる wtj プログラムのため処理をキャンセルします。")
                Me.CONSOLEWRITE("SyntacticAnalysys.Conversion : ", strPath)
                Return
            Else
                ' タイトル行の出力   ※カラム属性行・表示フラグ デフォルト：false
                If columnTitleFlg Then
                    '最初の行のみ、もしくはデバッグフラグ=1 の場合タイトル出力
                    If intTotalLineCount = 0 Or DBGFLG = 1 Then
            ...
            Try
                ...
                If c0_ViewID = "0" And c9_LinkColID = "0" Then
                    ...
                    ElseIf c5_RealValueType <> "0" Then
                        ...
                        Me.CONSOLEWRITE("925 CFValue", c2_Value)
...
#Region "Util"
    ...
    ''' -----------------------------------------------------------------------------------
    ''' <summary>
    '''  配列中から、にカンマ[,] があった場合、"comma" に置換する。
    '''  </summary>
    ''' <param name="key">
    '''     取り出す元になる文字列 配列</param>
    ''' -----------------------------------------------------------------------------------
    Private Function commaEscape(ByRef keyList As String())

        Dim foo As String
        For i As Integer = 0 To keyList.Length - 1
            foo = keyList(i)
            ' カンマ文字を全角の句読点に変換
            keyList(i) = foo.Replace(",", "、")
        Next
